...
@section('content')
							...
							<div class="row">
								<div class="col-md-6">
									<div class="detail-box">
										...
										<p>
											Di sini pengguna dapat melihat event apa saja yang akan/sedang/telah dilaksanakan
											oleh admin CGN. Jangan sampai ketinggalan informasi mengenai event kami!
										</p>
									</div>
								</div>
								<div class="col-md-6">
									<div class="img-box">
										<img src="{{ asset('User-depan/images/event_announcement.png') }}" alt="" />
									</div>
								</div>
							</div>
						</div>
    ...
    <div class="container1">
        <div class="left">
            @if($event->gambar)
            <img src="{{ asset('storage/' . $event->gambar) }}" alt="Gambar Modul" class="img-custom">
            @else
            <img src="{{ asset('User-depan/images/2022.png') }}" alt="Placeholder Gambar" class="img-custom">
            @endif
            <h5>Waktu Mulai</h5>
            <h6>{{ $event->waktu_mulai}}</h6>
            <h5>Waktu Selesai</h5>
            <h6>{{ $event->waktu_selesai}}</h6>
            <h5>Lokasi</h5>
            <h6>{{ $event->lokasi }}</h6>
            <!-- ->format('d F Y, H:i') -->
        </div>
        <div class="right">
            <h3>
                Event {{ ($events->currentPage() - 1) * $events->perPage() + $loop->iteration }}:
                {{ $event->nama }}
            </h3>

            <p class="pargarafmodul">{{ $event->deskripsi }}</p>
        </div>
    </div>
    <hr>
    @endforeach
    ...
    @if($events->isEmpty())
        <div class="container1">
            <h3>Events</h3>
            <h2>Tidak ada event tersedia</h2>
    ...
    <div class="pagination">
        {{ $events->appends(request()->input())->links('user.fitur.custom-pagination-view') }}
    </div>
