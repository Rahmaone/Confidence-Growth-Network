...
//             await fetchStreamToken(currentUser.id);
...
//     try {
//         const response = await axios.get(`/chat/getUserDetails/${userId}`);
...
// // Fungsi untuk mendapatkan token Stream
// async function fetchStreamToken(userId) {
//     try {
//         const response = await axios.get(`/chat/fetch-stream-token/${userId}`);
...
//         }
//     } catch (error) {
//         console.error("Failed to fetch Stream token:", error.message);
//         alert("Unable to fetch Stream token. Please try again later.");
//         return null;
//     }
// }
